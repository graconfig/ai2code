sap.ui.define(
  ...
  function (
    ...
    return Controller.extend(
      "ai.orchestration.taskrun.controller.TaskRunList",
      {
        onInit: function () {
        ...
        _createTask: function () {
          const sTaskName =
            sap.ui.core.Element.getElementById("taskName").getValue();
          const sTaskDescription =
            sap.ui.core.Element.getElementById("taskDescription").getValue();
          const sTaskTypeId =
            sap.ui.core.Element.getElementById("taskTypeId").getValue();

          const oModel = this.getOwnerComponent().getModel(); // OData V4 unbound action调用

          const oActionContext = oModel.bindContext(
            "/createTaskWithBots(...)",
            null,
            {
              parameters: {
          ...
          oActionContext
            ...
            .then(
              function () {
                ...
                if (oData && oData.ID) {
                  this._navToTaskRunDetail(oData.ID);
                }
              }.bind(this)
            )
            .catch(function (oError) {
              sap.m.MessageBox.error("Failed to create task");
            });
