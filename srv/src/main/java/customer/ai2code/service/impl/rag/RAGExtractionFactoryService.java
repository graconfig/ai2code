...
/**
 * RAGExtraction 工厂服务
 * 负责创建 RAGExtraction 实例并处理依赖注入
 */
@Service
public class RAGExtractionFactoryService {
    
    private final ApplicationContext applicationContext;
    
    // 预定义的包路径，用于搜索类
    private static final String[] SEARCH_PACKAGES = {
        "customer.ai2code.service.impl.rag",
        "customer.ai2code.service.rag.impl",
        "customer.ai2code.rag.extraction",
        // 可以根据需要添加更多包路径
    };

    public RAGExtractionFactoryService(ApplicationContext applicationContext) {
        this.applicationContext = applicationContext;
    }

    /**
     * 创建 RAGExtraction 实例 - 支持构造函数注入
     */
    public RAGExtraction createRAGExtractionInstance(String className) {
        try {
            Class<?> clazz = resolveClass(className);

            // 检查是否实现了 RAGExtraction 接口
            if (!RAGExtraction.class.isAssignableFrom(clazz)) {
                throw new BusinessException("Class " + className + " does not implement RAGExtraction interface");
            }

            // 方案1：优先尝试从 Spring 容器中获取已配置的 Bean
            try {
                if (applicationContext.getBeanNamesForType(clazz).length > 0) {
                    RAGExtraction instance = (RAGExtraction) applicationContext.getBean(clazz);
                    System.out.println("- Retrieved RAGExtraction instance from Spring container: " + clazz.getName());
                    return instance;
                }
            } catch (Exception e) {
            ...
            throw new BusinessException("Failed to create RAG extraction instance: " + className, e);
        }
    }
    ...
    private Class<?> resolveClass(String className) throws ClassNotFoundException {
        ...
        // 3. 尝试扫描 Spring 容器中的所有 RAGExtraction 实现类
        return findClassInSpringContext(className);
    }

    /**
     * 在 Spring 容器中查找 RAGExtraction 实现类
     */
    private Class<?> findClassInSpringContext(String className) throws ClassNotFoundException {
        try {
            // 获取所有 RAGExtraction 类型的 Bean
            String[] beanNames = applicationContext.getBeanNamesForType(RAGExtraction.class);
     ...
     * 手动创建实例并注入依赖
     */
    private RAGExtraction createInstanceWithDependencyInjection(Class<?> clazz) throws Exception {
        ...
        for (Constructor<?> constructor : constructors) {
            ...
            if (paramTypes.length > 0) {
                ...
                if (args != null) {
                    System.out.println("- Creating RAGExtraction instance with " + paramTypes.length + " constructor arguments: " + clazz.getSimpleName());
                    return (RAGExtraction) constructor.newInstance(args);
                }
            }
        }
        ...
        try {
            ...
            System.out.println("- Creating RAGExtraction instance with default constructor: " + clazz.getSimpleName());
            return (RAGExtraction) defaultConstructor.newInstance();
    ...
    /**
     * 获取 RAGExtraction 实例的详细信息
     */
    public String getRAGExtractionInstanceInfo(RAGExtraction instance) {
        if (instance == null) {
            return "RAGExtraction instance is null";
        }

        StringBuilder info = new StringBuilder();
        info.append("RAGExtraction Instance Info:\n");
