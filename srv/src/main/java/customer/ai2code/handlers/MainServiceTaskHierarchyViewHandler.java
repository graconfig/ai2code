...
import cds.gen.mainservice.TaskHierarchyView_;
import cds.gen.mainservice.TaskHierarchyView;

import static cds.gen.mainservice.MainService_.TASK_HIERARCHY_VIEW;


@Component
@ServiceName(MainService_.CDS_NAME)
public class MainServiceTaskHierarchyViewHandler implements EventHandler {
    private final PersistenceService db;

    MainServiceTaskHierarchyViewHandler(PersistenceService db) {
        this.db = db;
    }

    @Before(event = CqnService.EVENT_READ, entity = TaskHierarchyView_.CDS_NAME)
    public void readTaskHierarchyView(CdsReadEventContext event) {
        List<CqnTransformation> trafos = event.getCqn().transformations();
        List<TaskHierarchyView> result = null;
    ...
    private void setResult(CdsReadEventContext event, List<TaskHierarchyView> result) {
        if (!result.isEmpty()) {
    ...
    private CqnPredicate descendantsFilter(CqnDescendantsTransformation descendants) {
        ...
        CqnPredicate children = CQL.copy(start, new Modifier() {
            ...
            public CqnValue ref(CqnElementRef ref) {
                return CQL.get(TaskHierarchyView.PARENT_ID);
            }
        });
    ...
    private CqnPredicate ancestorsFilter(CqnAncestorsTransformation ancestors) {
        ...
        Select<TaskHierarchyView_> inner = Select.from(TASK_HIERARCHY_VIEW).columns(gh -> gh.ID());
        ...
        Select<TaskHierarchyView_> outer = Select.from(TASK_HIERARCHY_VIEW)
        ...
        return CQL.get(TaskHierarchyView_.ID).in(ancestorIds.stream().toList());
    }

    private List<TaskHierarchyView> handleDescendants(CqnDescendantsTransformation descendants) {
        CqnPredicate filter = descendantsFilter(descendants);
        CqnSelect childrenCQN = Select.from(TASK_HIERARCHY_VIEW).where(filter);
        List<TaskHierarchyView> nodes = db.run(childrenCQN).listOf(TaskHierarchyView.class);
    ...
    private static void connect(List<TaskHierarchyView> nodes) {
        Map<String, TaskHierarchyView> lookup = new HashMap<>();
    ...
    private List<TaskHierarchyView> handleAncestors(CqnAncestorsTransformation ancestors,
    ...
    private List<TaskHierarchyView> topLevels(CqnTopLevelsTransformation topLevels, CqnPredicate filter) {
        return topLevels.levels() < 0 ? topLevelsAll(filter) : topLevelsLimit(topLevels, filter);
    }

    private List<TaskHierarchyView> topLevelsLimit(CqnTopLevelsTransformation topLevels, CqnPredicate filter) {
        long limit = topLevels.levels();
        Map<String, TaskHierarchyView> lookup = new HashMap<>();
        Map<Object, Long> expandLevels = topLevels.expandLevels();

        CqnSelect getRoots = Select.from(TASK_HIERARCHY_VIEW).where(gh -> gh.parent_ID().isNull().and(filter));
        List<TaskHierarchyView> roots = db.run(getRoots).listOf(TaskHierarchyView.class);
        roots.forEach(root -> {
            ...
            for (long i = 1; i < limit; i++) {
                ...
                CqnSelect getChildren = Select.from(TASK_HIERARCHY_VIEW)
                        .where(gh -> gh.parent_ID().in(ps).and(filter));
                List<TaskHierarchyView> children = db.run(getChildren).listOf(TaskHierarchyView.class);
                if (children.isEmpty()) {
                    break;
                }
                long dfr = i;
                ...
                }).map(TaskHierarchyView::getId).toList();
            }
        });

        if (!expandLevels.isEmpty()) {
            List<String> expandedIds = expandLevels.keySet().stream().map(key -> (String) key).toList();
            CqnSelect expandedCQN = Select.from(MainService_.TASK_HIERARCHY_VIEW).where(gh -> CQL.and(filter,
                    CQL.or(gh.ID().in(expandedIds), gh.parent_ID().in(expandedIds))));

            List<TaskHierarchyView> expanded = db.run(expandedCQN).listOf(TaskHierarchyView.class);
            expanded.forEach(gh -> {
    ...
    private List<TaskHierarchyView> topLevelsAll(CqnPredicate filter) {
        CqnSelect allCqn = Select.from(TASK_HIERARCHY_VIEW).where(filter);
        var all = db.run(allCqn).listOf(TaskHierarchyView.class);
    ...
    private static long distanceFromRoot(TaskHierarchyView gh) {
        long dfr = 0;
    ...
    static class Sorter implements Comparator<TaskHierarchyView> {

        @Override
        public int compare(TaskHierarchyView gh1, TaskHierarchyView gh2) {
        ...
        Deque<String> getPath(TaskHierarchyView gh) {
